...
class Value:
    def __init__(self, data ,label= '', _childrens= [], _op= '' ,v= 0.0 , s=0.0):
        self.data = data
        self.label = label
...
class Loss:
    ...
    def momentum(self , mlp : MLP , beata1 : float) -> None:
        
        for n in mlp.parameters():
            n.v = n.v * beata1 + (1 - beata1) * n.grad
            n.data += -(self.lr * n.v)

    def RMSprop(self, mlp : MLP , beta2 : float , eps : float) -> None:

        for n in mlp.parameters():
            n.s = (beta2 * n.s) + (1 - beta2) * (n.grad **2)
            n.data += -(self.lr * n.grad)/(math.sqrt(n.s + eps))
