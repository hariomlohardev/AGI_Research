...
2. **Prior-strength sweep (the intuition plot):** Fix a small dataset (e.g. `data=[0.5, -0.3, 0.2]`, `prior_mu=5`). Vary `prior_sigma` from `10.0` (very weak) → `0.5` (very strong), compute `mu_map` each time and print (or `matplotlib` plot) the sequence. It must move monotonically from "≈ MLE" to "≈ prior" — this is your proof that the prior does what you think, and it's what `/done` will ask you to explain.
